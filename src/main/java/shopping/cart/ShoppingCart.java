...
public final class ShoppingCart
    ...
    ClusterSharding.get(system)
        .init(
            Entity.of(
                ENTITY_KEY,
                entityContext -> {
                  return ShoppingCart.create(entityContext.getEntityId(),cartEventTopic);
                }));
  }
  ...
  private ReplyEffect<Event, State> onAddItem(State state, AddItem cmd) {
    if (state.hasItem(cmd.itemId)) {
      ...
      ItemAdded event = new ItemAdded(cartId, cmd.itemId, cmd.quantity);
      return Effect()
          .persist(event)
          .thenRun(commonChainedEffect(event))
  ...
  private ReplyEffect<Event, State> onCheckout(State state, Checkout cmd) {
    if (state.isEmpty()) {
      ...
      CheckedOut event = new CheckedOut(cartId, Instant.now());
      return Effect()
          .persist(event)
          .thenRun(commonChainedEffect(event))
          .thenReply(cmd.replyTo, updatedCart -> StatusReply.success(updatedCart.toSummary()));
  ...
  private ReplyEffect<Event, State> onRemoveItem(State state, RemoveItem cmd) {
    if (state.hasItem(cmd.itemId)) {
      ItemRemoved event = new ItemRemoved(cartId, cmd.itemId, state.itemCount(cmd.itemId));
      return Effect()
          .persist(event)
          .thenRun(commonChainedEffect(event))
          .thenReply(cmd.replyTo, updatedCart -> StatusReply.success(updatedCart.toSummary()));
  ...
  private ReplyEffect<Event, State> onAdjustItemQuantity(State state, AdjustItemQuantity cmd) {
    if (cmd.quantity <= 0) {
      ...
      ItemQuantityAdjusted event = new ItemQuantityAdjusted(
